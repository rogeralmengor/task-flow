...
class Task(Base):
    ...
    current_segment_start = Column(
        DateTime, nullable=True
    )  # Start of current active period
...
class TimeTrackingDB:
    ...
    def add_task(self, agresso_code, activity, date=None):
        if date is None:
            date = datetime.now()
        task = Task(
            agresso_code=agresso_code, activity=activity, date=date, status="todo"
        )
    ...
    def get_running_task(self):
        ...
        return (
            self.session.query(Task)
            .filter(Task.status.in_(["running", "paused"]))
            .first()
        )
    ...
    def update_task_details(
        self, task_id, code, activity, started_at_iso, finished_at_iso
    ):
