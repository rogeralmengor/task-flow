...
class Task(Base):
    ...
    duration = Column(Float, default=0.0)  # Duration in hours
    status = Column(String(20), default='todo')  # todo, running, paused, finished
    started_at = Column(DateTime, nullable=True)
    paused_at = Column(DateTime, nullable=True)
    finished_at = Column(DateTime, nullable=True)
    paused_duration = Column(Float, default=0.0)  # Total paused time in hours
...
class TimeTrackingDB:
    ...
    def pause_task(self, task_id):
        ...
        if task and task.status == 'running':
            ...
            # Calculate elapsed time since start/unpause
            if task.started_at:
    ...
    def resume_task(self, task_id):
        ...
        if task and task.status == 'paused':
            task.status = 'running'
            task.started_at = datetime.now()  # Reset start time for next segment
            self.session.commit()
            return task
        return None
    ...
    def get_current_task_duration(self, task_id):
        """Get the current duration of a running/paused task"""
        task = self.get_task_by_id(task_id)
        if not task:
            return 0.0
        ...
        # Add current running time if the task is running
        if task.status == 'running' and task.started_at:
            elapsed = (datetime.now() - task.started_at).total_seconds() / 3600
            duration += elapsed
